...
parser.add_argument('--lambda_cl', type=float, default=0.1, help='weight for masked contrastive loss')
parser.add_argument('--cl_temp', type=float, default=0.2, help='temperature for contrastive InfoNCE')
...
def build_mask_view_graph(batch_graph, last_item, mask_id):
    """Deep copy the batch graph and mask the last item node ids with mask_id."""
    mask_graph = batch_graph.clone()
    item_counts = batch_graph.batch_num_nodes('item')
    item_offsets = torch.roll(torch.cumsum(item_counts, 0), 1)
    item_offsets[0] = 0
    global_indices = item_offsets.to(last_item.device) + last_item
    masked_item_ids = mask_graph.nodes['item'].data['item_id'].clone()
    masked_item_ids[global_indices] = mask_id
    mask_graph.nodes['item'].data['item_id'] = masked_item_ids
    return mask_graph
...
for epoch in range(opt.epoch):
    ...
    for user_id_tensor, user_alias, batch_graph, label, last_item in train_data:
        ...
        mask_graph = build_mask_view_graph(batch_graph, last_item, mask_id)
        score2, embedding2 = model(
            mask_graph,
            user_alias,
            last_item,
            is_training=True
        ...
        embedding1_norm = F.normalize(embedding1, dim=-1)
        embedding2_norm = F.normalize(embedding2, dim=-1)
        logits = torch.matmul(embedding1_norm, embedding2_norm.transpose(0, 1)) / opt.cl_temp
        cl_target = torch.arange(logits.size(0), device=device)
        cl_loss = F.cross_entropy(logits, cl_target)

        rec_loss = loss_func(score1, label)
        total_loss = rec_loss + opt.lambda_cl * cl_loss
        ...
        if iter % 400 == 0:
             print('Iter {}, loss {:.4f}, rec {:.4f}, cl {:.4f}'.format(
                iter, epoch_loss / iter, epoch_rec_loss / iter, epoch_cl_loss / iter
    ...
    if iter > 0:
        epoch_loss /= iter
        epoch_rec_loss /= iter
        epoch_cl_loss = epoch_cl_loss / iter
    else:
        epoch_loss = 0.0
        epoch_rec_loss = 0.0
        epoch_cl_loss = 0.0
